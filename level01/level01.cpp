...
level s_game_levels[]{
	{ Level00,  5, 2000, 10000, "level 00" },
	{ Level01,  5, 3000, 10000, "level 01" },
	{ Level02, 10, 2000, 10000, "level 02" },
	{ nullptr, 0, 0, 0, "" }
};
...
int main()
{
	...
	do {
		for (unsigned int i = 0; i < level->repeat; i++) {
			...
			std::this_thread::sleep_for(std::chrono::milliseconds(level->qst_time));
			user_total_qest++;
			system("CLS");
			
			PrintHead(level->name);
			correct_answ = PrintAnsw(start_time, end_time);
			//std::this_thread::sleep_for(std::chrono::milliseconds(level->answ_time));
